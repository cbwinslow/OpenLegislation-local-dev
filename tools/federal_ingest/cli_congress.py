"""Command line interface for the congress.gov REST client."""
...
LOGGER = logging.getLogger("federal_ingest.congress")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Ingest data from the congress.gov REST API")
    parser.add_argument("collection", help="Collection to retrieve, e.g. 'bill' or 'member'")
    parser.add_argument("--api-key", dest="api_key", help="Override the CONGRESS_API_KEY environment variable")
    parser.add_argument("--limit", type=int, help="Maximum number of records to fetch")
    parser.add_argument("--page-size", dest="page_size", type=int, default=250, help="Number of items per page (<=250)")
    parser.add_argument(
        "--param",
        action="append",
        default=[],
        metavar="KEY=VALUE",
        help="Additional query parameter to send to the API. Can be provided multiple times.",
    )
...
def parse_params(pairs: Iterable[str]) -> Dict[str, str]:
    ...
    for pair in pairs:
        if "=" not in pair:
            raise argparse.ArgumentTypeError(f"Parameters must be KEY=VALUE pairs: {pair}")
...
def main(argv: List[str] | None = None) -> int:
    ...
    client = CongressClient(api_key=args.api_key)
    params = parse_params(args.param)
    records = list(client.iter_collection(args.collection, limit=args.limit, page_size=args.page_size, **params))
    LOGGER.info("Fetched %s records from congress.gov", len(records))
