...
class TestBillIngestionIntegration:
    ...
    def test_end_to_end_ingestion(self, test_db_conn, mock_xml_dir):
        ...
        ingestor = GovInfoBillIngestor(
            xml_dir=mock_xml_dir, db_config=settings.test_db_config
        )
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM master.bill WHERE bill_print_no = 'H1' AND congress = 119"
        )
        ...
        # Verify sponsor
        cursor.execute(
            "SELECT sponsor_name FROM master.bill_sponsor WHERE bill_id = (SELECT bill_id FROM master.bill WHERE bill_print_no = 'H1')"
        )
        ...
        # Verify action
        cursor.execute(
            "SELECT description FROM master.bill_amendment_action WHERE bill_print_no = 'H1' AND bill_session_year = 119"
        )
    ...
    def test_error_handling_and_rollback(self, test_db_conn, mock_xml_dir, mocker):
        ...
        mocker.patch.object(
            ingestor, "_insert_bill_data", side_effect=Exception("DB Error")
        )
    ...
    def test_data_validation_and_uniqueness(self, test_db_conn, mock_xml_dir):
        ...
        invalid_record = {
            "record_id": "INVALID",
            "metadata": {"xml_file": mock_xml_dir + "/invalid.xml"},
        }
        with patch("builtins.open", mock_open(read_data="<invalid>")):
            success = ingestor.process_record(invalid_record)
        assert success is False
        cursor.execute(
            "SELECT COUNT(*) FROM master.bill WHERE bill_print_no = 'INVALID'"
        )
        count = cursor.fetchone()[0]
        assert count == 0
    ...
    def test_foreign_key_integrity(self, test_db_conn):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM master.bill_sponsor WHERE bill_id = %s", (bill_id,)
        )
        ...
        conn.commit()
        cursor.execute(
            "SELECT COUNT(*) FROM master.bill WHERE bill_id = %s", (bill_id,)
        )
