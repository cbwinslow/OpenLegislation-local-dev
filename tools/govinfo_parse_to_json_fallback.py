...
def extract_sponsors(text):
    ...
    for p in [
        ...
        for m in re.finditer(p, text, flags=re.IGNORECASE | re.DOTALL):
            ...
            name = None
            mm = re.search(
                r"<name[^>]*>(.*?)</name>", inner, flags=re.IGNORECASE | re.DOTALL
            )
            if mm:
...
def extract_actions(text):
    ...
    for m in re.finditer(
        r"<action([^>]*)>(.*?)</action>", text, flags=re.IGNORECASE | re.DOTALL
    ):
        attr = m.group(1)
        inner = m.group(2)
        # date attribute
...
def parse_file(path):
    ...
    title = first_match(
        text,
        [
            r"<official-title[^>]*>(.*?)</official-title>",
            r"<officialTitle[^>]*>(.*?)</officialTitle>",
            r"<title[^>]*>(.*?)</title>",
        ],
    )
    short_title = first_match(
        text,
        [
    ...
    introduced = first_match(
        text,
        [
            r"<introduced-date[^>]*>(.*?)</introduced-date>",
            r"<introducedDate[^>]*>(.*?)</introducedDate>",
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: govinfo_parse_to_json_fallback.py /path/to/samplesdir",
            file=sys.stderr,
        )
        sys.exit(2)
    in_dir = sys.argv[1]
