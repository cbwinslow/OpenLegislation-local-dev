...
def fetch_all_members(congress: int, chamber: str = None, api_key: str = None) -> List[Dict]:
    """
...
def main():
    parser = argparse.ArgumentParser(description="Fetch and map Congress members from API")
    parser.add_argument('--congress', type=int, default=118, help='Congress number (default: 118)')
    parser.add_argument('--chamber', choices=['senate', 'house', 'joint'], help='Filter by chamber')
    parser.add_argument('--api_key', default=os.getenv('CONGRESS_API_KEY'), help='API key (or set CONGRESS_API_KEY env)')
    parser.add_argument('--output', default=None, help='Output JSON file (default: members_{congress}_{chamber}.json)')
    parser.add_argument('--details', action='store_true', help='Fetch full details for each member (slower)')
    ...
    print(f"Fetching members for Congress {args.congress}, chamber: {args.chamber or 'all'}")
