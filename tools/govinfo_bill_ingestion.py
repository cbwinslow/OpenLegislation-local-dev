...
class GovInfoBillIngestor(BaseIngestionProcess):
    ...
    def __init__(self, xml_dir: Optional[str] = None, **kwargs):
        ...
        self.xml_dir = xml_dir or getattr(settings, 'govinfo_xml_dir', '/tmp/govinfo_xml')
        ...
        try:
            ...
            for action_elem in action_elems:
                ...
                if text_elem is not None:
                    actions.append({
                        'action_date': action_date_elem.text if action_date_elem is not None else "",
                        'chamber': chamber_elem.text if chamber_elem is not None else "",
                        'description': text_elem.text,
                        'action_type': code_elem.text if code_elem is not None else ""
                    })
