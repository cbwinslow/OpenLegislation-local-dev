...
from tools.govinfo_pydantic_models import (
    GovInfoBill,
    GovInfoSponsor,
    GovInfoAction,
    GovInfoBillText,
...
def parse_govinfo_bill(xml_text: str) -> GovInfoBill:
    ...
    bill_num = (
        root.findtext(".//billNumber")
        or root.findtext(".//legislativeIdentifier")
        or "UNKNOWN"
    )
    ...
    for t in root.findall(".//textVersions/textVersion"):
        ...
        texts.append(
            GovInfoBillText(
                version_id=vid,
                html=content if fmt == "html" else None,
                plain_text=content if fmt != "html" else None,
            )
        )
