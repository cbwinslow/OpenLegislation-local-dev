...
class ProgressStats:
    ...
    def get_overall_progress(self) -> Dict[str, Any]:
        ...
        total_downloaded = sum(
            c.get("files_downloaded", 0) for c in self.collections.values()
        )
        total_bytes = sum(
            c.get("bytes_downloaded", 0) for c in self.collections.values()
        )
...
class ProgressTUI:
    ...
    def _draw_screen(self):
        ...
        line = 4
        self.screen.addstr(
            line, 2, f"Files Found: {progress['total_files_found']:,}", curses.A_DIM
        )
        line += 1
        self.screen.addstr(
            line,
            2,
            f"Files Downloaded: {progress['total_files_downloaded']:,}",
            curses.A_DIM,
        )
        line += 1
        self.screen.addstr(
            line,
            2,
            f"Data Downloaded: {progress['total_bytes_downloaded'] / (1024**3):.2f} GB",
            curses.A_DIM,
        )
        line += 1
        self.screen.addstr(
            line,
            2,
            f"Download Speed: {progress['download_rate_bps'] / (1024**2):.2f} MB/s",
            curses.A_DIM,
        )
        line += 1
        self.screen.addstr(
            line, 2, f"Elapsed Time: {progress['elapsed_time']}", curses.A_DIM
        )
        line += 1
        self.screen.addstr(
            line, 2, f"ETA: {progress['estimated_time_remaining']}", curses.A_DIM
        )
        ...
        if "error" not in system:
            self.screen.addstr(
                line, 2, f"CPU: {system['cpu_percent']:.1f}%", curses.A_DIM
            )
            line += 1
            self.screen.addstr(
                line,
                2,
                f"Memory: {system['memory_used_gb']:.1f}/{system['memory_total_gb']:.1f} GB ({system['memory_percent']:.1f}%)",
                curses.A_DIM,
            )
            line += 1
            self.screen.addstr(
                line,
                2,
                f"Disk: {system['disk_used_gb']:.1f}/{system['disk_total_gb']:.1f} GB ({system['disk_percent']:.1f}%)",
                curses.A_DIM,
            )
        else:
            self.screen.addstr(
                line, 2, f"System monitoring error: {system['error']}", curses.A_DIM
            )
        ...
        if self.stats.errors or self.stats.warnings:
            ...
            issues = (self.stats.errors[-3:] + self.stats.warnings[-3:])[
                -5:
            ]  # Last 5 issues
            for issue in issues:
        ...
        self.screen.addstr(
            height - 1, (width - len(help_text)) // 2, help_text, curses.A_DIM
        )
...
def print_progress_summary():
    ...
    print(
        f"Files: {progress['total_files_downloaded']:,} / {progress['total_files_found']:,}"
    )
