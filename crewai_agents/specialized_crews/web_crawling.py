...
class DatabaseIngestionAgent(Agent):
        ...
        try:
            ...
            with orchestrator.get_db_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    self._start_crawl_session(cursor, session_id, processed_data)
                    ...
                    for data_type, items in processed_data.items():
                        ...
                        inserted = self._insert_crawled_data_batch(
                            cursor, data_type, items,
                            processed_data['metadata']['jurisdictions'][0] if processed_data['metadata']['jurisdictions'] else 'unknown'
                        )
                        results['inserted_records'] += inserted
                        print(f"Inserted {inserted} {data_type} records")

                    # Update crawl session as completed
                    self._complete_crawl_session(cursor, session_id, results['inserted_records'])
    ...
    def _start_crawl_session(self, cursor, session_id: str, processed_data: Dict[str, Any]):
        """Start a crawl session in the database"""

        cursor.execute("""
        INSERT INTO crawl_sessions (
            session_id, total_websites, jurisdictions_crawled, data_types_crawled
        ) VALUES (%s, %s, %s, %s)
        """, (
            session_id,
            len(processed_data.get('metadata', {}).get('jurisdictions', [])),
            json.dumps(processed_data.get('metadata', {}).get('jurisdictions', [])),
            json.dumps(processed_data.get('metadata', {}).get('data_types', []))
        ))

    def _complete_crawl_session(self, cursor, session_id: str, total_records: int):
    ...
    def _insert_crawled_data_batch(self, cursor, data_type: str, items: List[Dict], jurisdiction: str) -> int:
        """Insert a batch of crawled data items"""
        ...
        for item in items:
            try:
                ...
                cursor.execute("""
                INSERT INTO crawled_legislative_data (
                    jurisdiction, data_type, source_url, content_hash, raw_data
                ) VALUES (%s, %s, %s, %s, %s)
                """, (
                    jurisdiction,
                    data_type,
                ...
                print(f"Error inserting {data_type} item: {e}")
                continue
    ...
    def _trigger_ingestion_pipelines(self, processed_data: Dict[str, Any]) -> List[str]:
        ...
        # The database triggers will automatically handle ingestion based on the data inserted
        # We just need to mark records as processed to trigger the database functions
        try:
            from tools.enhanced_ingestion_orchestrator import EnhancedIngestionOrchestrator
            ...
            with orchestrator.get_db_connection() as conn:
                with conn.cursor() as cursor:
                    # Mark all newly inserted records as processed to trigger ingestion
                    cursor.execute("""
                    UPDATE crawled_legislative_data
                    SET processing_status = 'processed', processed_at = CURRENT_TIMESTAMP
                    WHERE processing_status = 'pending'
                    """)
                    conn.commit()

                    # Check what triggers were activated
                    for data_type in data_types:
                        jurisdiction = processed_data.get('metadata', {}).get('jurisdictions', [None])[0]
                        if jurisdiction:
                            triggered_jobs.append(f"{jurisdiction}_{data_type}_ingestion")
